...
export async function routeConsultation({ query, business_context = "" }) {
  ...
  return {
    ...
    next_step: "Call load_knowledge_bundle for every selected skill and continue until next_cursor is null before synthesizing advice."
  };
}
...
export async function loadKnowledgeBundle({ skill_ids, cursor, page_size_chars = DEFAULT_PAGE_SIZE }) {
  const catalog = await loadKnowledgeCatalog();
  ...
  const nextCursor = end < totalCharacters
    ? encodeCursor({ digest: catalog.digest, skill_ids: selectedSkillIds, offset: end })
    : null;
